  ...
  <div class="app-container">
      ...
      <el-form ref="queryFormRef" :model="queryParams" :inline="true">
        ...
        <el-form-item label="反馈状态" prop="status">
          <el-select v-model="queryParams.status"  placeholder="全部" clearable class="!w-[100px]">
            <el-option :value="1" label="已回答" />
            <el-option :value="0" label="待处理" />
          </el-select>
        </el-form-item> 
        <el-form-item label="问题类型" prop="workOrderType">
          <dict-select v-model="queryParams.workOrderType" code="workOrderType" placeholder="请选择问题类型"></dict-select>
        </el-form-item> 
    ...
    <el-card shadow="never">
      ...
      >
        ...
        <el-table-column prop="context" label="问题" min-width="200" />
        <el-table-column label="问题类型" width="100" align="center">
          <template #default="scope">
            <!-- 身份类型字典翻译 -->
            <DictLabel v-model="scope.row.workOrderType" code="workOrderType" />
          </template>
        </el-table-column>
        <el-table-column prop="applyDate" label="申请日期" width="180" align="center">
          <template #default="scope">
            <span>{{ formatApplyDate(scope.row.applyDate) }}</span>
          </template>
        </el-table-column>

        <!-- <el-table-column prop="createBy" label="创建者" min-width="200" /> -->
        <el-table-column prop="status" label="反馈状态" width="100">
          <template #default="scope">
            <el-tag v-if="scope.row.status == 1" type="success">已回答</el-tag>
...
import issueAPI,{QueryParams} from "@/api/system/client/issue";
import {formatApplyDate}  from '@/utils/datedisplay';
import themeEdit from '@/views/school/workOrder/edit.vue'
...
function handleQuery() {
  ...
  issueAPI.getPage(params).then((data:any) => {
